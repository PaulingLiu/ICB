...
for (i in 1:length(cts)) {
  ...
  a <- t.test(tmp.mp$f[tmp.mp$group != "post.NR"], tmp.mp$f[tmp.mp$group == "post.NR"])
  
  b <- esc_t(t = a$statistic,     
             grp1n = length(tmp.mp$f[tmp.mp$group != "post.NR"]), 
...
get.es <- function(.x){
  ...
  for (i in 1:length(cts)) {
    ...
    a <- t.test(tmp.mp$prop[tmp.mp$group != "post.NR"], tmp.mp$prop[tmp.mp$group == "post.NR"])
    
    b <- esc_t(t = a$statistic,     
               grp1n = length(tmp.mp$prop[tmp.mp$group != "post.NR"]), 
